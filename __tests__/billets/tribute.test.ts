...
describe("Test tribute billet creation", () => {
  ...
  it("Create invalid billet", () => {
    expect(() => TributeBillet.createBillet(123456789123)).toThrowError("Value must be bellow 999999999.99")
  })
})

describe("Test tribute billet parser", () => {
  it("Invalid barcode", () => {
    expect(() => TributeBillet.parseBillet("10800000001230000000000000000000000000000000")).toThrowError(
      "Invalid barcode"
    )
    expect(() => TributeBillet.parseBillet("80800000001230000000000000000000000000000000")).toThrowError(
      "Invalid barcode"
    )
  })

  it("Invalid line", () => {
    expect(() => TributeBillet.parseBillet("108000000014230000000003000000000000000000000000")).toThrowError(
      "Invalid line"
    )
    expect(() => TributeBillet.parseBillet("808000000014230000000003000000000000000000000000")).toThrowError(
      "Invalid line"
    )
  })

  it("Billet format error", () => {
    expect(() => TributeBillet.parseBillet("8086000000123000000xxx0000000000000000000000")).toThrowError(
      "Billet format error"
    )
  })
